...
func TestServeShowSupersedeRetract(t *testing.T) {
	...
	id3, err := toolSupersede(context.Background(), s, json.RawMessage(`{"ref":"`+id2+`","fact":"v3","session":"s3","inherit":true,"subject":""}`))
	require.NoError(t, err)
	cleared, err := toolSearch(context.Background(), s, json.RawMessage(`{"query":"v3"}`))
	require.NoError(t, err)
	require.Contains(t, cleared, `"subject":""`)
	require.Contains(t, cleared, `"tags":["infra"]`)
	_, err = toolRetract(context.Background(), s, json.RawMessage(`{"ref":"`+id3+`","session":"s4"}`))
	require.NoError(t, err)
	hits, err := toolSearch(context.Background(), s, json.RawMessage(`{"query":"v3"}`))
	require.NoError(t, err)
